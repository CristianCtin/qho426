...
def coffee_sleep():
  ...
  for i in range (7):
    ...
    sleep.append(int(input("Enter the aveage sleep length: ")))
  return coffee, sleep
...
def music():
  songs = {}
  for i in range(7):
    song = input("Enter your favorite movie song: ")
...
def MM_vs_DD():
  ...
  dd = 0
  for i in range(7):
    ans = input("Who wins? Miky Mouse of Donald Duck ")
    if ans == "DD":
      dd += 1
    elif ans.upper() == "MM":
      mm += 1
...
def run():
  ...
  ax3.pie(ss.values(), labels = ss.keys(), autopct = "%1.1%%")
